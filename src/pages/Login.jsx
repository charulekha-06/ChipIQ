...
import { HiOutlineWrench, HiOutlineChartBar, HiOutlineChip, HiOutlineShieldCheck } from 'react-icons/hi';

const ROLES_INFO = {
  engineer: { icon: <HiOutlineWrench size={18} />, label: 'VERIFICATION ENGINEER', tabs: 6, color: '#D32F2F', bg: 'rgba(211, 47, 47, 0.1)' },
  ...
  const handleEnroll = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setEnrollStatus(`❌ ${data.detail}`);
      }
    } catch {
      setEnrollStatus('❌ Cannot reach backend server.');
    }
    ...
    <div className="face-login-wrapper">
      <div className="face-camera-section">
        <div className="face-frame" style={{ borderColor }}>
          ...
          {/* Success overlay */}
          {faceStatus === 'success' && (
            <div className="face-success-overlay">✓</div>
          )}
        </div>
      ...
      <div className="enroll-section">
        ...
        {enrollMode && (
          <div className="enroll-panel">
            ...
            {enrollStatus && (
              <p className={`enroll-status ${enrollStatus.startsWith('✅') ? 'ok' : 'err'}`}>
                {enrollStatus}
              </p>
            )}
          </div>
              ...
              {role && ROLES_INFO[role] && (
                <div className="role-pill" style={{ backgroundColor: ROLES_INFO[role].bg, border: `1px solid ${ROLES_INFO[role].color}` }}>
                  <div className="role-pill-left">
                    ...
                    <span style={{ color: ROLES_INFO[role].color, fontFamily: 'monospace', fontWeight: 600 }}>
